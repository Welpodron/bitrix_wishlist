(function(window){
	...
	window.JCCatalogElement.prototype = {
		...
		init: function()
		{
			...
			// Добавление объектов связанных с механизмом добавления в список желаемого 
			if (this.obProduct) {
				...
				// 
				// Инициализации первой попавшейся кнопки-иконки состояния всего html документа (подразумеваем, что она только одна)
			...
			if (this.errorCode === 0)
			{
				...
				// Добавление обработчика для блока с избранным
				if (this.obDelay) {
		...
		initConfig: function()
		{
			...
			// Добавлен параметр конфигурации для сравнения
			this.config.useDelay = this.params.CONFIG.DISPLAY_DELAY;
			// 
		...
		// Добалвена инициализация функции добалвения в избранное
		initDelayData: function() {
			if (this.config.useDelay)
			{
				if (this.params.DELAY && typeof this.params.DELAY === 'object')
				{
					...
					if (this.params.DELAY.DELAY_DELETE_URL_TEMPLATE)
					{
						this.delayData.delayDeleteUrl = this.params.DELAY.DELAY_DELETE_URL_TEMPLATE;
					}
					else
					{
						this.config.useDelay = false;
		...
		// Добавлена функция добавления в избранное
		delay: function(event) {
			...
			if (url)
			{
				...
				BX.ajax({
					...
					url: delayLink + (delayLink.indexOf('?') !== -1 ? '&' : '?') + 'ajax_action=Y',
					onsuccess: checked
		...
		setDelayed: function(state)
		{
			...
			var checkbox = this.getEntity(this.obDelay, 'delay-checkbox');
			if (checkbox)
			{
		...
		delayDeleteResult: function()
		{
			console.log("delayDeleteResult");
		},
		// 
		...
		wishlistRunToggleResult: function(result) {
			...
			if (result && result.OK) {
				this.wishlistSetChecked(result.WISHLISTED);
				this.wishlistSetCounter(result.WISHLIST_COUNTER);
			} else {
				console.error(result);
			}
			// API FIX
		...
		// 
		// Прототип: функция(метод) wishlistSetCounter
